...
# ~/CIS-LBK/functions/recommendations/nix_fed_ensure_nftables_not_enabled.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       10/07/20    Recommendation "Ensure nftables is not enabled"
# 
fed_ensure_nftables_not_enabled()
{
	...
	if [ "$FWIN" != "FWd" ]; then
		...
		if ! $PQ firewalld; then
			...
			if ! systemctl status nftables | grep -q 'Active: active (running) '; then
				test2=passed
			fi
			if [ "$test1" = passed ] && [ "$test2" = passed ]; then
				test=passed
			else
				# Disable and mask the nftables service
