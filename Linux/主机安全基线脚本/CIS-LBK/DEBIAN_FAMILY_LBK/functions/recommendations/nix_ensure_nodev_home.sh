...
ensure_nodev_home()
{
	...
	if mount | grep -Eq '\s\/home\s'; then
		if grep -Eq '^\s*[^#]+\s+/home\s+' /etc/fstab; then
			if ! grep -E '^\s*[^#]+\s+\/home\s+' /etc/fstab | grep -vq 'nodev'; then
				test=passed
			else
				sed -ri 's/^\s*([^#]+\s+\/home\s+)(\S+\s+)(\S+)?(\s+[0-9]\s+[0-9].*)$/\1\2\3,nodev\4/' /etc/fstab
				mount -o remount,noexec,nodev,nosuid /home
				! grep -E '^\s*[^#]+\s+\/home\s+' /etc/fstab | grep -vq 'nodev' && test=remediated
			fi
		else
			test=manual
		fi
