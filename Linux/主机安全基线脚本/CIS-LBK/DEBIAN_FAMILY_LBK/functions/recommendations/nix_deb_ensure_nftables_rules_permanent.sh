...
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       11/20/20    Recommendation "Ensure nftables rules are permanent"
# 
deb_ensure_nftables_rules_permanent()
{
	...
	if [ "$FWIN" != "NFt" ]; then
		...
		if ! $PQ nftables >/dev/null; then
			...
			if grep -Eqs "^\s*include" /etc/nftables.conf && awk '/hook input/,/}/' "$(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' /etc/nftables.conf)"; then
				test1=passed
			fi

			# Test for hook forward (test2)
			if grep -Eqs "^\s*include" /etc/nftables.conf && awk '/hook forward/,/}/' "$(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' /etc/nftables.conf)"; then
				test2=passed
			fi

			# Test for hook output (test3)
			if grep -Eqs "^\s*include" /etc/nftables.conf && awk '/hook output/,/}/' "$(awk '$1 ~ /^\s*include/ { gsub("\"","",$2);print $2 }' /etc/nftables.conf)"; then
				test3=passed
			fi
			...
			if [ "$test1" = passed ] && [ "$test2" = passed ] && [ "$test3" = passed ]; then
				...
				if [ -s /etc/nftables/cis_nftables.rules ]; then
					echo "include \"/etc/nftables/cis_nftables.rules\"" /etc/nftables.conf
				else
