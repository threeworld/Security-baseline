...
ensure_sctp_disabled()
{
	...
	if modprobe -n -v sctp | grep -E '(sctp|install)' | grep -Eq "^\s*install\s+\/bin\/(true|false)\b"; then
		test1=passed
	else
		echo "install sctp /bin/true" >> /etc/modprobe.d/sctp.conf
		modprobe -n -v sctp | grep -E '(sctp|install)' | grep -Eq "^\s*install\s+\/bin\/(true|false)\b" && test1=remediated
	fi
	...
	if ! lsmod | grep -Eq "^\s*sctp\b"; then
		...
		rmmod sctp
		! lsmod | grep -Eq "^\s*sctp\b" && test2=remediatet
	fi
