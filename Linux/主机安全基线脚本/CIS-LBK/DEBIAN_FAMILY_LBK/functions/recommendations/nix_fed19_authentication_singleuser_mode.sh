...
# ~/CIS-LBK/functions/recommendations/nix_fed19_authentication_singleuser_mode.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       09/16/20    Recommendation "Ensure authentication required for single user mode"
# 
fed19_authentication_singleuser_mode()
{
	test=""
	test1=""
	test2=""
	# Test rescue.service
	if grep -Eq '^\s*ExecStart\s*=\s*-\/bin\/sh\s+-c\s+"(\/usr)?\/sbin\/sulogin;\s+\/usr\/bin\/systemctl\s+(--fail|--job-mode=fail)\s+--no-block default"' /usr/lib/systemd/system/rescue.service; then
		test1=passed
	else
		...
		grep -Eq '^\s*ExecStart\s*=\s*-\/bin\/sh\s+-c\s+"(\/usr)?\/sbin\/sulogin;\s+\/usr\/bin\/systemctl\s+(--fail|--job-mode=fail)\s+--no-block default"' /usr/lib/systemd/system/rescue.service && test1=remediated
	fi
	...
	if grep -Eq '^\s*ExecStart=-/usr/lib/systemd/systemd-sulogin-shell\s+emergency' /usr/lib/systemd/system/emergency.service; then
		...
		if grep -Eq '^\s*ExecStart\s*=\s*-\/bin\/sh\s+-c\s+"(\/usr)?\/sbin\/sulogin;\s+\/usr\/bin\/systemctl\s+(--fail|--job-mode=fail)\s+--no-block default"' /usr/lib/systemd/system/emergency.service; then
		...
		grep -Eq '^\s*ExecStart\s*=\s*-\/bin\/sh\s+-c\s+"(\/usr)?\/sbin\/sulogin;\s+\/usr\/bin\/systemctl\s+(--fail|--job-mode=fail)\s+--no-block default"' /usr/lib/systemd/system/emergency.service && test2=remediated
	fi                                                   
