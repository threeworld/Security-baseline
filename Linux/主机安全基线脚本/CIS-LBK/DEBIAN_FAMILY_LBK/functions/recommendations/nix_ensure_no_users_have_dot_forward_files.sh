...
# ~/CIS-LBK/functions/recommendations/nix_ensure_no_users_have_dot_forward_files.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       10/09/20    Recommendation "Ensure no users have .forward files"
# 
ensure_no_users_have_dot_forward_files()
{
	echo "- $(date +%d-%b-%Y' '%T) - Starting $RNA" | tee -a "$LOG" 2>> "$ELOG"
	test=""	output="" dir=""
	XCCDF_VALUE_REGEX=".forward"
