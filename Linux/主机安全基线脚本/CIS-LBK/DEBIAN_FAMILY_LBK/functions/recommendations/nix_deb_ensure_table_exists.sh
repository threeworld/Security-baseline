...
# ~/CIS-LBK/functions/recommendations/nix_deb_ensure_table_exists.sh
# 
# Name                Date       Description
# ------------------------------------------------------------------------------------------------
# Eric Pinnell       11/20/20    Recommendation "Ensure a table exists"
# 
deb_ensure_table_exists()
{
	echo "- $(date +%d-%b-%Y' '%T) - Starting $RNA" | tee -a "$LOG" 2>> "$ELOG"
	create_nft_rule_file()
	{
		echo '#!/sbin/nft -f' > /etc/cis_nftables.rules
		{
			...
			echo '   }'
			echo '}'
		} >> /etc/cis_nftables.rules
		# Load the created file into nftables
		nft -f /etc/cis_nftables.rules
		# Update to make the rules file active on boot
		echo "include \"/etc/cis_nftables.rules\"" >> /etc/nftables.conf
	}
	test=""
...
#				nft create table inet filter
				# No NFTables table exists, create and load NFTables rules file /etc/cis_nftables.rules
				create_nft_rule_file
